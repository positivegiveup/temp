...
# 新增可視化策略的函數
def visualize_policy(policy, max_cars, title="Jack's Car Rental Policy Heatmap"):
    """
    Visualize the policy matrix as a heatmap.
    
    Parameters:
    - policy: 2D numpy array, the policy matrix where policy[s1, s2] is the action to take.
    - max_cars: Maximum number of cars (defines the size of the matrix).
    - title: Title of the heatmap.
    """
    plt.figure(figsize=(10, 8))
    # 使用 seaborn 的 heatmap 繪製策略矩陣
    sns.heatmap(policy, cmap="RdBu", center=0, annot=True, fmt="d", 
                cbar_kws={'label': 'Action (Cars Moved)'},
                xticklabels=range(max_cars + 1),
                yticklabels=range(max_cars + 1))
    
    # 設置標籤和標題
    plt.xlabel("Cars at Location 2 (s2)")
    plt.ylabel("Cars at Location 1 (s1)")
    plt.title(title)
    plt.show()
